...
\end{tabular}
\end{table}
In comparison with Khanggsar, the Gyurong dialect presents both innovative and conservative  features. The loss 
...
The first and second person pronouns (\ipa{ŋa} \textsc{1sg}, \ipa{ɲə} \textsc{2sg}) do not have ergative forms (except in hybrid indirect speech, see  \ref{sec:reported}), as illustrated by example \ref{ex:qow}, where the noun \ipa{waqi-w}  `the rabbit' takes the ergative while the pronoun \ipa{ŋa} remains invariable in exactly the same context.
 \begin{exe}
\ex \label{ex:qow}
\gll
...
%
%vʚ sə ɮdə ja, ɣrə de ə́-ɮdə mí-ɮdə χa mi-gõ
%
%
%ŋa zəŋʚ qe-ŋkʰə ŋõ ŋõ va ŋõ
...
\end{exe}
\ipa{ŋa} 	\ipa{zɮdə-lə} 	\ipa{ŋgʚ} 	\ipa{kə-ftsu} 


The locative nominalizer \ipa{--re} can be used in combination with the verb \ipa{ɮdi} to express the meaning `be ready to, have almost ...', as in \ref{ex:stere}.
\begin{exe}
\ex \label{ex:stere}
\gll 
...
\ex \label{ex:EdEgi}
\gll
		[\ipa{ədə-gi} 	\ipa{mbjoˠmbjoˠ} 	\ipa{tɕa} 	\ipa{gə} 	\ipa{tə-f-kʰʚ-ã,} 	\ipa{tɕa} 	\ipa{gə} 	\ipa{tə-f-kʰʚ-ã,}] 	\ipa{jə-rə} 	\\
...
\end{exe}
 %{TAM and evidentiality} \label{sec:TAM}
...
%ɲi ŋa-gi kə-ma-ʁʚ ve, ŋa liska kʰərma gə və mí-vzu-rə
...
%	You are  bigger 
%	
%	liska ɕə-ʁa nəŋʚve, ɲchara təɕã ve sʚ scə
%	Ce serait plus sympa que j'aille me promener plutôt que d'aller travailler
%	
%jitʰa ndzʚ-ʁa nə-ŋʚ-ve, smẽba qʰi tə-ɕə ve, sʚ xtɕʰi ŋə-rə	
%	
%	ŋa ɲi nəŋʚve, liska ɕã
...
\section{Classification} \label{sec:classification}
 ...
 An example of such a trace is the noun \ipa{ɣɟi}  `hole, orifice' in Stau,  \ipa{ʁɟô} in Wobzi Khroskyabs, both   cognate to Japhug \ipa{--ɣɲɟɯ}  `(its) hole, opening', which  derives from \ipa{ɲɟɯ} `be opened (intr)', the anticausative of \ipa{cɯ} `open (a door)' (\citealt[4-6]{jacques14antipassive}). 
 
 In Japhug, \ipa{--ɣ} is an irregular allomorph of the nominalization prefix \ipa{kɯ--}, found in a handful of examples. Although the \ipa{ɣ--} element in Stau and \ipa{ʁ--} in Khroskyabs are not analyzable anymore (the forms \ipa{ɣɟi}  and   \ipa{ʁɟô} are synchronically unmotivated), this example and others (see \citealt[1228-9]{jacques12incorp}) suggest  that nominalization prefixes used to exist in Stau and Khroskyabs, and were later replaced by innovative suffixes.  
...
Stau and Khroskyabs have two distinct roots for `year', one with the numeral prefixes (one year etc) and the other in year ordinals (last year, this year, next year etc).

Wobzi Khroskyabs \ipa{--dju} corresponds to Thugsrjechenbo \ipa{--dɣu} and goes back to a cluster *\ipa{dgu}
  \begin{table}[h]
